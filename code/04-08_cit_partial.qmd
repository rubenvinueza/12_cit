...
Conditional inference tree is a recursive model that works iteratively performing 2 steps:  
  - **variable selection**  (select a predicted variable based on the predicting variable)
  - **binary split**  
  These two steps happen multiple times when the model is running. 
...
Terminology:  
  ...
  - **Leaf/terminal node**: the bottom nodes with the boxplot (most important to remember because here you can see how your predictable variable was affected)
...
There will be 2 main hyperparameters that we will fine-tune:  
  - **maximum depth**: maximum (vertical) depth of the tree (how many horizontal layers stack between each other)
  - **minimum criterion**:  the value of (1 - p-value) that must be exceeded in order to implement a split. (it has to be significant in order to split, if its out of range in p-value it would not)
     
## Pros vs. cons of CIT  
Pros:  
  - Non-parametric (does not assume an specific distribution of the data, not assumption resudials or data normality)
  - It can model non-linear relationships  
  - The model created is a decision tree, very easy to interpret  
  - Can be used with both numerical and categorical response variables  (in case of elastic net it only works with numerical variables)
  
Cons:  
  - Can have higher bias  
  - Potentially lower predictive power: any observation matching a given condition will be predicted as the mean of the terminal node (will you mean of the box plot)
...
#install.packages("partykit") implements conditional trees
#install.packages("finetune") find tune of this specific model
#install.packages("bonsai") find tune of this specific model
...
```{r weather_train}
# Setting train set (pulling out our training data)
...
```{r weather_test}
# Setting test split (treat the test data)
...
```{r distribution}
#distribution of train and test data, because we did stratified sampling we see a normal distribution
ggplot() +
...
```{r weather_recipe}
weather_recipe <-
  # Defining predicted and predictor variables (setting the y variable)
...
weather_recipe
```
steps are not applied to my data yet. 
...
First, let's specify:  
  ...
  - which **mode** we want to use  (regresion (numerica) or clasification(categorical))
...
Conditional inference tree **hyperparameters**:  
  ...
  - **mincriterion**: the value of 1 - p-value that must be exceeded in order to implement a split  (is the inverse of p value)
...
A given model type can be fit with different engines (e.g., through different packages). Here, we'll use the **partykit** engine/package. (The engine is what R package can do that analysis)
  
```{r cit_spec}
cit_spec <- 
  # Specifying cit as our model type, asking to tune the hyperparameters
  decision_tree(tree_depth = tune()) %>% #tune will say what I want to tune
    # Specify the engine
  set_engine("partykit", 
...
cit_grid_result <- tune_sim_anneal(
  ...
  iter = 50 
) # in research is recommended a 100 for time thing is 50 in the class
...
> Previously, we selected the single best model. Now, let's select the best model within one std error of the metric, so we choose a model among the top ones that is more parsimonious. (to get a simple model which not lose predictive power)
...
It makes sense because our model will predict better in the data you trained. 
...
```
Two only variables showed up. Anything else does not have any variable of importance because they were the only ones who enter into the model
